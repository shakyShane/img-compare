...
var transforms  = require("./arg-transforms");
//var args        = path.join(__dirname, "pdiff");
var args        = "/Users/shakyshane/pdiff/perceptualdiff";
...
module.exports = function (files, config, cb) {
    ...
    cmdopts.output = utils.makeOutPath(config.cwd, config.output);

    var cmd = [
        args,
        utils.makePaths(files, config.cwd),
        utils.mergeCmdOpts(cmdopts, config, whitelist, transforms)
    ].join(" ");
